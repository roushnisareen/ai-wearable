...
import torch.quantization  # For dynamic quantization on CPU

# Import Groq and base64 for the vision/text processing
...
class VoiceToVoicePipeline:
    def __init__(
        ...
        # Initialize Groq client for vision and text queries
        self.groq_client = Groq(api_key="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")
    ...
    def is_vision_query(self, text):
        ...
        for pattern in self.compiled_patterns:
            if pattern.search(text):
                ...
                print(f"Matched vision pattern: {pattern.pattern}")
                return True
        self.timing_info['match_time'] = time.perf_counter() - start_time
        return False
    ...
    def process_image(self, image_path):
        """
        Process the image using the Groq vision model to generate a textual description.
        """
        start_time = time.perf_counter()
        ...
        # Create the message payload for Groq with the image
        messages = [
            {
                "role": "user",
                "content": [
        ...
        # Call the Groq API to get the description
    ...
    def process_text(self, text):
        """
        Process text query using the Groq model.
        """
        start_time = time.perf_counter()
        messages = [
            {
    ...
    def handle_audio_input(self, audio_path, output_audio_path="output"):
        ...
        if is_vision:
            ...
            # 4. Process the captured image using Groq
            ...
            # Process as general text by sending it to Groq
